...
const THEMES = [
  {id:"night",    name:"Ніч",       base:"dark",  bg:"#050505", panel:"#0d0d0e",
   line:"#26262a", accent:"#40e094", up:"#40e094", down:"#ff6e60", be:"#efc258"},
  {id:"graphite", name:"Графіт",    base:"dark",  bg:"#101012", panel:"#17171a",
   line:"#2c2c31", accent:"#f0a13c", up:"#59c98a", down:"#ef6a5e", be:"#e0b352"},
  {id:"midnight", name:"Опівніч",   base:"dark",  bg:"#070b14", panel:"#0d1421",
   line:"#22304a", accent:"#38bdf8", up:"#34d399", down:"#fb7185", be:"#fbbf24"},
  {id:"wine",     name:"Вино",      base:"dark",  bg:"#120a12", panel:"#1b101c",
   line:"#352036", accent:"#f472b6", up:"#6ee7b7", down:"#fb7185", be:"#fcd34d"},
  {id:"pine",     name:"Хвоя",      base:"dark",  bg:"#06110d", panel:"#0c1a14",
   line:"#1b3328", accent:"#a3e635", up:"#4ade80", down:"#f87171", be:"#facc15"},

  {id:"day",      name:"День",      base:"light", bg:"#f2f4f8", panel:"#ffffff",
   line:"#dde3ec", accent:"#2b62e3", up:"#128a53", down:"#c94430", be:"#9c7514"},
  {id:"paper",    name:"Папір",     base:"light", bg:"#f6f2ea", panel:"#fffdf8",
   line:"#e3dbcc", accent:"#c2410c", up:"#15803d", down:"#b91c1c", be:"#a16207"},
  {id:"fog",      name:"Туман",     base:"light", bg:"#eef1f5", panel:"#ffffff",
   line:"#d9dfe7", accent:"#475569", up:"#0f766e", down:"#be123c", be:"#a16207"},
  {id:"sand",     name:"Пісок",     base:"light", bg:"#f4f1e8", panel:"#fffefa",
...
function card(t, on){
  return '<button class="th-card' + (on ? " on" : "") + '" onclick="__skin.set(\'' + t.id + '\')">'
    + swatch(t)
    + '<div class="nm">' + esc(t.name) + (on ? "<i>обрана</i>" : "") + "</div></button>";
}
...
function customCard(on){
  ...
  return '<button class="th-card' + (on ? " on" : "") + '" onclick="__skin.set(\'custom\')">'
    ...
    + '<div class="nm">Своя' + (on ? "<i>обрана</i>" : "") + "</div></button>";
}
...
function draw(){
  ...
  const h = '<div class="m-head"><h2>Оформлення</h2>'
    + '<button class="x" onclick="closeModal()">×</button></div>'
    + '<div class="m-body"><div class="nt th-grp">'
    + group("Темні",  THEMES.filter(t => t.base === "dark"))
    + group("Світлі", THEMES.filter(t => t.base === "light"))
    + '<div class="nt-sub">Своя</div>'
    + '<div class="th-grid">' + customCard(now === "custom") + "</div>"
    + '<div class="th-custom">'
    +   '<div class="th-pick"><span>Основа</span><div class="th-base">'
    +     '<button class="' + (s.base === "dark" ? "on" : "") + '" onclick="__skin.seed(\'base\',\'dark\')">Темна</button>'
    +     '<button class="' + (s.base === "light" ? "on" : "") + '" onclick="__skin.seed(\'base\',\'light\')">Світла</button>'
    +   "</div></div>"
    +   '<div class="th-pick"><span>Тло</span>'
    +     '<input type="color" value="' + s.bg + '" oninput="__skin.seed(\'bg\', this.value)"></div>'
    +   '<div class="th-pick"><span>Акцент</span>'
    +     '<input type="color" value="' + s.accent + '" oninput="__skin.seed(\'accent\', this.value)"></div>'
    +   '<p class="nt-note" style="flex:1 1 180px">Два кольори — решту підберемо: '
    +     "підкладки, лінії й підсвітку.</p>"
    + "</div></div></div>"
