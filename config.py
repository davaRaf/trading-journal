...
# Фонові справи: щоденний зліпок журналу і перечитування Notion. Робити їх
# має рівно одна копія сайту. Поки копія одна — вмикати нічого не треба,
# а коли їх стане дві, другій ставимо RUN_JOBS=0, інакше зліпок робитиметься
# двічі й Notion читатиметься двічі.
RUN_JOBS      = os.environ.get("RUN_JOBS", "1") not in ("0", "false", "no")
# Скільки одночасних відвідувачів сайт обслуговує, не ставлячи в чергу.
# Робітник переважно чекає (база, Gemini), а не рахує, тому їх більше, ніж ядер.
WEB_WORKERS   = int(os.environ.get("WEB_WORKERS", 64))
# Скільки з'єднань з базою тримаємо відкритими. Більше за WEB_WORKERS сенсу
# немає — стільки запитів одночасно все одно нікому виконувати.
