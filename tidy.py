...
def plain(value):
    ...
    остаются разными. По этому ключу сделка приводится к общему написанию
    при записи (db.py): чужой пробел можно подставить молча, а решить, что
    USTEC — тот же индекс, что NAS100, должен человек — для этого `key`
    и окно сведения.
    """
    return _plain(value)
