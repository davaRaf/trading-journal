...
RULES = (
    ...
    "Нижче виписка з журналу цього трейдера. Числа й угоди бери ЛИШЕ звідти: нічого "
    "не рахуй заново і не вигадуй угод, яких там немає. Якщо у виписці потрібного "
    "немає — скажи це одним реченням і все одно допоможи по суті: поясни, порадь, "
    "підкажи, що варто записувати далі.\n"
...
def ask(user_id, question, history=None):
    ...
    prompt = "%s\n\nВИПИСКА З ЖУРНАЛУ:\n%s%s\n\nПИТАННЯ ТРЕЙДЕРА: %s\n\n%s" % (
        RULES, book, _history_block(history), question, lang_order(question))
    return llm.ask(prompt, max_tokens=900) or \
...
def review(user_id, history=None):
    ...
    if not facts:
        return {"facts": [], "text": ""}
    text = llm.ask(
        "Ти — спокійний тренер з трейдингу. Нижче — те, що видно в журналі трейдера. "
        "Це факти, вони вже пораховані: спирайся тільки на них.\n\n%s\n\n"
        "До 4 речень: скажи головне, що варто виправити, і одну "
        "конкретну дію. Без вступів, без списків, без співчуття.\n\n%s"
        % ("\n".join("- " + f for f in facts), _lang_hint(history)), max_tokens=700)
